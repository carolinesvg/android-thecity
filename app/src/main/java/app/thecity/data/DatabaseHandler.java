...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public void insertListPlaceAsync(List<Place> modelList) {
        ...
        for (Place p : modelList) {
            ...
            if (p.address != null) stmt.bindString(4, p.address);
            else stmt.bindNull(4);
            if (p.phone != null) stmt.bindString(5, p.phone);
            else stmt.bindNull(5);
            if (p.website != null) stmt.bindString(6, p.website);
            else stmt.bindNull(6);
            if (p.description != null) stmt.bindString(7, p.description);
